...
public class Sea{
  ...
  *@return Cell
  */
  public Cell getCellAtPos(Position p){
    ...
    public void addShipVertically(Ship ship,Position p)throws IllegalStateException{
      if ((p.getX()>this.width||p.getY()>this.height)||(p.getY()+ship.getLength()>this.height)){
        ...
        for(int i=p.getY();length-->0;i++){
          ...
          if (this.getCellAtPos(p1).getShip()!=null)throw new IllegalStateException("can't place this ship at this position");
          else{
            this.getCellAtPos(p1).setShip(ship);
          }
        }
        }
      }
    ...
    public void display(boolean defender){
      for (int i=0;i<this.height;i++){
        for (int j=0;j<this.width;j++){
          System.out.print(this.grid[i][j].getCharacter(defender));
        }
        System.out.print("\n");
      }
    }
